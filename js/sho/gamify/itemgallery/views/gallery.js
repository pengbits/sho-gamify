    ...
    @author				dpaul
    @modified		    04.11.13
    @desc		        View class for the Item Gallery component.
    -------------------------------------------------------------------------------------------- */  

    sho.provide('sho.gamify.itemgallery.views');

    /**
     * class sho.gamify.itemgallery.views.Gallery
     * View class for the Item Gallery component.
     * 
     *  1. startup
     *      1. handlers
     *      2. render
    ...
    **/
    ...
    sho.gamify.itemgallery.views.Gallery = Backbone.View.extend({

        '_KLASS_' : 'sho.gamify.itemgallery.views.Gallery',
        ...
        setHandlers:function()
        {
            ...
            var th=this; sho.string.toArray(['',
                ...
                'onScrollTo paginate redirectToChallenge redeemItem'
            ...
            this.container.on('click', '.btn.get-started.start-challenge', this.fn.redirectToChallenge);
            
            // these are new, added for redemption
            this.container.on('click', '.btn.redeem', this.fn.redeemItem);
            this.container.on('click', '.btn.get-started.redeem-item', this.fn.redeemItem)
        ...
        render:function()
        {
            var data =      this.model.get('items'),
                view =      sho.gamify.itemgallery.views.Item,
                th=         this
            ;
			
			this.container.removeClass('loading');
			
			// Check to see if there are items to display
			if(data.length > 0) {						
				...
				// Render message if no items available
				this.container.html("<p style='padding: 18px 20px 26px 20px;font-size: 14px;line-height: 24px;'>Sorry, there are no Red Rewards items currently available. Please check back soon.</p>");
			}
        },
        ...
        renderRadialProgressWidgets:function(el)
        {
            ...
            sho.gamify.itemgallery.renderIcon(el);
        },
        ...
        setDetail:function(el)
        {
            this.controller.setDetail({
                'name' : unescape(sho.dom.data.read(el, 'challengeName')),
                'id'   : sho.dom.data.read(el, 'challengeId')
            })
        },
        
        renderDetail:function(attrs)
        {
            var el =        this.expandedChallengeInnerElement(),
                view =      sho.gamify.itemgallery.views.Details,
                templ =     _.extend(attrs, {
                    'isGamified' : sho.accounts.isGamified,
                }),
                partials =  {
                    'challengeProgress' : sho.gamify.itemgallery.getRadialProgressView('expanded'),
                    'share' : sho.social.components.view
                }
            ;
            
            el.html(Mustache.to_html(view, templ, partials));
        ...
        {   
            ...
            // was sho.gamify.itemgallery.renderSocialComponents(this.expandedChallengeInnerElement());
        },
        ...
        /**
         * sho.gamify.itemgallery.views.Gallery#expandedChallengeElement() -> jQuery
        ...
        **/
        ...
        redeemItem:function(e)
        {
            sho.dom.trap(e);
            
            if($(e.currentTarget).hasClass('is-inactive')) return
            this.controller.redeemItem(sho.dom.data.read(e.currentTarget, 'itemId'))
        }
